...
export class QueryChampionshipDto {
    ...
    @IsIn(QueryChampionshipDto.VALID_RELATIONS, {
        each: true,
        message: (args) => {
            return `Relacion/es: ${args.value} inválida/s. Relacion/es permitida/s: ${QueryChampionshipDto.VALID_RELATIONS.join(', ')}`;
        },
    })
    include?: string[];
}
